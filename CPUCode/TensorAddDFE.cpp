...
double* readFirstRankTensor(int dim) {
...
void writeFirstRankTensor(int dim, double* resultTensor) {
	for (int ind = 0; ind < dim; ind++) {
		output << resultTensor[ind] << " ";
	}
}

double* readSecondRankTensor(int dim1, int dim2) {
	double* d = new double[dim1*dim2];
...
void writeSecondRankTensor(int dim1, int dim2, double* resultTensor) {
	for (int ind1 = 0; ind1 < dim1; ind1++) {
		for (int ind2 = 0; ind2 < dim2; ind2++) {
...
double* readThirdRankTensor(int dim1, int dim2, int dim3) {
...
void writeThirdRankTensor(int dim1, int dim2, int dim3, double* resultTensor) {
	for (int ind1 = 0; ind1 < dim1; ind1++) {
		for (int ind2 = 0; ind2 < dim2; ind2++) {
			for (int ind3 = 0; ind3 < dim3; ind3++) {
...
void tensorAddDFE() {
	...
	double* inTensor1=0, *inTensor2=0, *resultTensor=0;
	...
	case 2:
		...
		resultTensor = new double[dim1*dim2];
	...
	case 3:
		...
		resultTensor = new double[dim1*dim2*dim3];
